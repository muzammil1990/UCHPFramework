...
module uchpClientAngularApp {
  ...
  class ClaimJobItemCostTransactionController {
    private fresh: any;
    private form: any;
    ...
    constructor(private ClaimJobItemStore: IClaimJobItemStore,
      ...
      that.claimJobSubscription = that.DispatcherService.subscribe('claimJob:getClaimJobItemCostTransactionTab',
        that.claimJobResultHandler.bind(this));
      that.$scope.$on('$stateChangeStart', function () {
        that.claimJobSubscription.unsubscribe();
      });
      if (that.fresh) {
        console.log('new claim Cost Transaction');
      }
    }
  ...
  angular.module('uchpClientAngularApp')
    .component('claimJobItemCostTransaction', {
      ...
      controller: [
        ...
        'NewClaimService',
        'ClaimJobStore',
        ClaimJobItemCostTransactionController
      ],
      controllerAs: 'ctrl',
      templateUrl: 'components/claim/job/claim-job-item/claim-job-item-cost-transaction/claim-job-item-cost-transaction.html'
    });
}
