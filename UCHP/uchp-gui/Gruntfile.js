...
module.exports = function (grunt) {
  ...
  grunt.initConfig({
    ...
    // TODO: config correctly
    // https://libraries.io/npm/grunt-angular-translate
    i18nextract: {
      default_options: {
        src: [ '<%= yeoman.client %>/{app,components}/**/!(*.spec|*.mock).ts',
          '<%= yeoman.client %>/{app,components}/**/!(*.spec|*.mock).html' ],
        lang:     ['fr_FR'],
        dest:     'tmp'
      }
    },
  ...
  */
    ...
    ngconstant: {
      options: {
        name: 'uchpClientAngularApp.constants',
    ...
    //edit index.html to set contextRoot for JBoss
    'string-replace': {
      prod: {
        src: '<%= yeoman.dist %>/<%= yeoman.client %>/index.html',
        dest: '<%= yeoman.dist %>/<%= yeoman.client %>/index.html',
        options: {
          replacements: [{
            pattern: '/uchp/client',
            replacement: '/uchp/client/'
          }]
        }
      }
    },
    ...
    copy: {
      dist: {
        files: [{
          ...
          expand: true,
          cwd: '<%= yeoman.client %>',
          dest: '<%= yeoman.dist %>/<%= yeoman.client %>/src',
          src: ['generated/*']
        }, {
          expand: true,
          dest: '<%= yeoman.dist %>',
    ...
    // TODO issue registered on github for addition of typings_test.json config
    typings: {
      install: {}
    },
      ...
      scripts: {
        ...
        files: {
          '<%= yeoman.client %>/index.html': [
               [
                 '<%= yeoman.client %>/{app,components}/**/!(*.spec|*.mock|*_all).ts',
